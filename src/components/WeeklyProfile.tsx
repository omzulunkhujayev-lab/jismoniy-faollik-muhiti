...
export const WeeklyProfile: React.FC<WeeklyProfileProps> = ({ entries, goals, onAddGoal }) => {
  ...
  const [activeDayReason, setActiveDayReason] = useState(
    lang === 'ru' 
      ? "Вторник и Четверг — было много пеших переходов между аудиториями."
      : lang === 'en'
      ? "Tuesday and Thursday — lots of walking between classrooms."
      : "Seshanba va Payshanba – auditoriyalararo piyoda ko'chishlar ko'p bo'ldi."
  );
  const [lowDayReason, setLowDayReason] = useState(
    lang === 'ru'
      ? "Среда — весь день готовился в библиотеке непрерывно сидя."
      : lang === 'en'
      ? "Wednesday — studied continuously sitting in the library."
      : "Chorshanba – kutubxonada uzluksiz o'tirib tayyorlandim."
  );
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-slate-700 shadow-xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-gray-100 dark:border-slate-700">
          ...
          <div className="px-4 py-2 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 text-xs font-bold flex items-center gap-2">
            ...
            <span>{lang === 'ru' ? '28 Августа – 3 Сентября 2026' : lang === 'en' ? 'Aug 28 – Sep 3, 2026' : '28-Avgust – 3-Sentabr 2026'}</span>
          </div>
        </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-800 space-y-3">
            <div className="flex justify-between items-start">
              ...
              <div className={`px-2.5 py-1 rounded-full text-xs font-bold ${stepsPercentage >= 100 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'}`}>
                {stepsPercentage}% {lang === 'ru' ? 'нормы' : lang === 'en' ? 'norm' : 'me\'yor'}
              </div>
            </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-2xl font-extrabold text-teal-600 dark:text-teal-400">
                  {totalActiveMins} {lang === 'ru' ? 'мин' : lang === 'en' ? 'mins' : 'daqiqa'}
                </div>
              </div>
              <div className={`px-2.5 py-1 rounded-full text-xs font-bold ${activeMinsPercentage >= 100 ? 'bg-teal-100 text-teal-700 dark:bg-teal-950 dark:text-teal-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'}`}>
                {activeMinsPercentage}% {lang === 'ru' ? 'нормы' : lang === 'en' ? 'norm' : 'me\'yor'}
              </div>
            </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className={`text-2xl font-extrabold ${maxSitting > 60 ? 'text-rose-500' : 'text-emerald-600'}`}>
                  {maxSitting} {lang === 'ru' ? 'мин' : lang === 'en' ? 'mins' : 'daqiqa'}
                </div>
              </div>
              <div className={`px-2.5 py-1 rounded-full text-xs font-bold ${maxSitting <= 60 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300'}`}>
                {maxSitting <= 60 
                  ? (lang === 'ru' ? 'Безопасно' : lang === 'en' ? 'Safe' : 'Xavfsiz') 
                  : (lang === 'ru' ? 'Красная зона!' : lang === 'en' ? 'Red zone!' : 'Qizil zona!')}
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-slate-700 shadow-xl space-y-6">
        ...
        {reflectionSubmitted && (
          <div className="p-4 rounded-2xl bg-emerald-500 text-white font-medium text-xs sm:text-sm flex items-center gap-3">
            ...
            <span>{lang === 'ru' ? 'Рефлексия сохранена.' : lang === 'en' ? 'Reflection saved.' : 'Refleksiya saqlandi.'}</span>
          </div>
        )}
        ...
        <div className="space-y-4 text-xs">
          <div>
            <label className="font-bold text-gray-800 dark:text-gray-200 block mb-1">
              {lang === 'ru' ? '1. Какой день был самым активным и почему?' : lang === 'en' ? '1. Which day was most active and why?' : '1. Qaysi kun eng faol bo\'ldi va nima uchun?'}
            </label>
            <textarea
              rows={2}
          ...
          <div>
            <label className="font-bold text-gray-800 dark:text-gray-200 block mb-1">
              {lang === 'ru' ? '2. В какой день зафиксирован наименьший показатель и почему?' : lang === 'en' ? '2. Which day recorded the lowest metric and why?' : '2. Qaysi kun eng past ko\'rsatkich qayd etildi, sababi nimada?'}
            </label>
            <textarea
              rows={2}
              value={lowDayReason}
        ...
        <form onSubmit={handleGoalSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2">
              <div className="text-xs text-slate-400">{lang === 'ru' ? 'Текущий средний показатель:' : lang === 'en' ? 'Current average metric:' : 'Joriy o\'rtacha ko\'rsatkich:'}</div>
              <div className="text-2xl font-extrabold text-white">
                {avgSteps.toLocaleString()} {lang === 'ru' ? 'шагов' : lang === 'en' ? 'steps' : 'qadam'}
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-200">
                {lang === 'ru' ? 'Цель на следующую неделю:' : lang === 'en' ? 'Goal for next week:' : 'Kelgusi hafta uchun maqsad:'}
              </label>
